-- Helpers present have have been copied and slightly modified from:
--
...
Helpers.expect.equality_approx = MiniTest.new_expectation('approximate equality', function(x, y, tol)
...
Helpers.new_child_neovim = function()
  ...
  -- - `load_*` - load with "normal" table config
  -- - `load_strconfig_*` - load with "string" config, which is still a
  ...
  --   through RPC).
  -- - `unload_*` - unload module and revert common side effects.
  child.load_lzd = function(config)
    child.lua([[require('lazydocker').setup(...)]], { config })
  end

  child.load_strconfig_lzd = function(strconfig)
    local t = {}
    ...
    local command = ([[require('lazydocker').setup(%s)]]):format(str)
